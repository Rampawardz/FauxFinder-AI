...
function Dashboard() {
  ...
  const [result, setResult] = useState(null);          // analysis result (no DB _id)
  const [alerts, setAlerts] = useState([]);
  const [loading, setLoading] = useState(false);
  const [isReportOpen, setIsReportOpen] = useState(false);
  const [aiOpen, setAiOpen] = useState(false);

  // will hold the profileId of the profile that was analyzed/stored in DB
  const [analyzedProfileId, setAnalyzedProfileId] = useState(null);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // backend should return both analysis and the saved profileId
      // e.g. { riskScore, isFake, profileId, ... }
      const data = res.data;
      setResult(data);
      if (data.profileId) {
        setAnalyzedProfileId(data.profileId);
      }
    } catch (err) {
      setResult({ error: err.response?.data?.error || "Analysis failed" });
    }
  ...
  const handleSubmitReport = async (reason, severity) => {
    // use the stored profileId from the analysis response
    const profileId = analyzedProfileId;
    if (!profileId) {
      alert("Profile reference missing for reporting. Please analyze again.");
      return;
    }

    try {
      const token = localStorage.getItem("token");
      await createReport(profileId, reason, severity, token);
      alert("Profile reported successfully!");
    } catch (e) {
      alert("Failed to report: " + (e.message || "Unknown error"));
    }
  ...
  return (
    <section className="w-screen min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-orange-50 via-rose-50 to-yellow-50 pt-32 pb-16 px-6 space-y-8">
      ...
      <ReportModal
        ...
        onClose={() => setIsReportOpen(false)}
        onSubmit={handleSubmitReport}
        profile={null} // backend now uses analyzedProfileId instead
      />

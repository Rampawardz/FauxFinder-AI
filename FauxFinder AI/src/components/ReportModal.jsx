...
function ReportModal({ isOpen, onClose, onSubmit, profile, analysis }) {
  const [reason, setReason] = useState("");
  const [severity, setSeverity] = useState("low");
  ...
  const handleSubmit = () => {
    ...
    // no dependency on analysis/profile for submission
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50 backdrop-blur-sm bg-black/40">
      <div className="bg-white/30 backdrop-blur-lg border border-white/40 shadow-xl rounded-2xl p-6 w-96 text-gray-800">
        ...
        {analysis && analysis.riskScore !== undefined && (
          <p className="text-xs text-gray-700 mb-3 text-center">
            Current risk score:{" "}
            <span className="font-semibold text-red-600">
              {(analysis.riskScore * 100).toFixed(2)}% fake
            </span>
          </p>
        )}
